#Max Walker's Cricket Game. A conversion of the popular 80's dice cricket game
#Copyright (C) 2026 Tony Francis. 
...
class CricketGUI:
    ...
    def _current_innings_num(self) -> int:
        """Return the innings number currently being displayed / played (1-4)."""
        try:
            # Prefer Match state if available
            if hasattr(self, "match") and self.match is not None:
                mi = getattr(self.match, "current_innings", None)
                if isinstance(mi, int) and 1 <= mi <= 4:
                    self.current_innings = mi
            ci = getattr(self, "current_innings", 1)
    ...
    def _flush_end_of_match_ui(self):
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                self.update_batting_chart(self.match._batting_team() if getattr(self,'match',None) else [])
                self.update_bowling_chart(self.match._bowling_team() if getattr(self,'match',None) else [])
            except Exception:
                pass
...
# -------------------- SAVE TEAM --------------------
    ...
    def select_innings_tab(self, innings_num: int) -> None:
        ...
        try:
            if hasattr(self,'match') and self.match is not None:
                self.match.current_innings = innings_num
        except Exception:
            pass
        try:
